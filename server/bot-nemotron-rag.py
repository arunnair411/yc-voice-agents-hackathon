...
# Fixed preamble for the injected catalog block. Used both as the message
# content prefix and as the marker for stripping the previous turn's injection.
RAG_CONTEXT_PREFIX = (
    "The following catalog information was retrieved specifically for the "
    "caller's latest message. Use it as a helpful hint when recommending "
    "bouquets, but always verify stock with check_availability before "
    "committing.\n\n"
)
        ...
        if isinstance(frame, LLMContextFrame) and direction == FrameDirection.DOWNSTREAM:
            ...
            if user_text:
                ...
                if rag_context:
                    ...
                    rag_message = {"role": "system", "content": RAG_CONTEXT_PREFIX + rag_context}

                    # Rebuild the message list: drop any RAG block injected on a
                    # previous turn (otherwise stale catalog hints accumulate every
                    # turn), then splice the fresh one in immediately before the
                    # last user message. LLMContext.add_message() only appends — it
                    # has no `index` arg — so we edit the list and set_messages().
                    messages = [
                        m
                        ...
                        if not (
                            ...
                            and m["content"].startswith(RAG_CONTEXT_PREFIX)
                        )
                    ]
