...
"""Field & Flower — flower shop voice ordering bot with RAG (rag branch).

This is a drop-in replacement for ``bot-gpt.py`` that augments the LLM with
...
items and injects them as additional context.  Everything else — transport,
STT, TTS, tools, Twilio wiring — is identical to the original.

Pipeline:
    Gradium STT → (RAG lookup) → OpenAI Responses LLM → Gradium TTS

How RAG is wired in:
    ``RAGContextProcessor`` sits between the user aggregator and the LLM.
    On every ``LLMMessagesFrame`` it extracts the latest user message, runs
    ``rag.get_rag_context()``, and injects the result as an extra system-level
    message so the LLM sees it alongside its normal system prompt.
...
class RAGContextProcessor(FrameProcessor):
    """Intercepts LLMMessagesFrame, runs a RAG lookup, and injects the
    retrieved catalog context as a system message before passing the frame on.

    This keeps the RAG logic completely decoupled from the LLM service and
    tool functions — it's just another stage in the Pipecat pipeline.
    """
    ...
    async def process_frame(self, frame, direction):
        ...
        if isinstance(frame, LLMMessagesFrame) and direction == FrameDirection.DOWNSTREAM:
            # Extract the last user message to use as the RAG query.
            ...
            if user_text:
                ...
                if rag_context:
                    ...
                    # Inject as a system-role message immediately before the
                    # user turn so it's fresh in the LLM's context window.
                    augmented_messages = list(messages)
                    # Find position of last user message and insert just before it
                    insert_at = len(augmented_messages)
                    ...
                    augmented_messages.insert(
                        insert_at,
                        {
                            ...
                            "content": (
                                ...
                                "for the caller's latest message. Use it to inform your "
                                "response, but still rely on your tools for stock checks and "
                                "ordering.\n\n" + rag_context
                            ),
                        },
                    )
                    frame = LLMMessagesFrame(messages=augmented_messages)

        await self.push_frame(frame, direction)


# ---------------------------------------------------------------------------
# Twilio helper (unchanged from original)
# ---------------------------------------------------------------------------
...
async def run_bot(
    ...
    """Main bot logic — identical tool set to bot-gpt.py, plus RAG."""
    ...
    system_instruction = (
        ...
        # RAG-specific note added here:
        "You will sometimes receive a block of 'Relevant Catalog Information' retrieved "
        "from our catalog database. Use this as a helpful hint when recommending bouquets, "
        "but always verify stock with check_availability before committing.\n\n"
    ...
    # --- Services (identical to bot-gpt.py) ---------------------------------
...
# ---------------------------------------------------------------------------
# Entry points (identical structure to bot-gpt.py)
# ---------------------------------------------------------------------------
